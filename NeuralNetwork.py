...
class reseau_de_neurones():
    def __init__(self, optimizer = "SDG"):
        self.optim = optimizer
        self.layers = []

    def ajouter_couche(self, number_of_entries, number_of_neurons, activation_function):
    ...
    def __repr__(self):
        ...
        if (len(self.layers) > 0):
            layers += "Nombre d'entrées dans le réseau: " + str(self.layers[0].weights.shape[0]) + "\n\n"
        for couche in self.layers:
            layers += "Couche N°" + str(self.layers.index(couche) + 1) + ": " + str(couche.weights.shape[1]) + " neurones,  Fonction d\'activation: "
            if couche.activation_function != None:
                layers += " " + couche.activation_function + "\n"
            else:
                layers += " " + "Il n\'y en a pas" + "\n"
        return layers
